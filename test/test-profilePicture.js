...
describe('Matches Endpoint', function () {
  ...
  describe('/api/main/profilePicture', function () {
    describe('GET /api.main/profilePicture', function () {

      it('Should reject requests with no credentials', function () {
        return chai
          .request(app)
          .get(`/api/main/profilePicture/${user._id}`)
          .then((res) => {
      ...
      it('should return profilePicture', function () {
        return chai
          .request(app)
          .get(`/api/main/profilePicture/${user._id}`)
          .set('Authorization', `Bearer ${token}`)
          .then((res) => {
            ...
            expect(res.body.profilePicture).to.equal('picture.jpeg');
          });
      });
    });
    describe('POST /api/main/profilePicture', function () {
      const body = {
        profilePic: 'newPicture.jpeg'
      };
      it('Should reject requests with wrong id', function () {
        const token = jwt.sign(
          {
        ...
        return chai
          .request(app)
          .post(`/api/main/profilePicture/${user._id}`)
          .set('authorization', `Bearer ${token}`)
          .send(body)
          .then((res) => {
            expect(res).to.have.status(401);
      ...
      it('should update profile picture', function () {
        return chai
          .request(app)
          .post(`/api/main/profilePicture/${user._id}`)
          .set('authorization', `Bearer ${token}`)
          ...
          .then((res) => {
            ...
            expect(res.body.profilePicture).to.equal(body.profilePic);
          });
      });
    });
  });
